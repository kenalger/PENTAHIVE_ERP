-- admin_create_user — create an auth.users row + matching identity from the
-- admin UI without needing the create-user Edge Function.
-- SECURITY DEFINER: runs with postgres-level privileges, but checks the caller
-- has the 'admin' role before doing anything. Returns the email + generated
-- temp password to display once. The new user is forced to change the password
...
begin
  ...
  -- 3. Duplicate check.
  if exists (select 1 from auth.users where lower(email) = v_clean) then
