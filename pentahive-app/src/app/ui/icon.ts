...
/**
 * Inline SVG icon component — Lucide glyphs, zero runtime dependency.
 *
 * Why inline (not a font / not lucide-angular):
 *  - Strokes inherit `currentColor`, so icons recolor with their context
 *    (ink in light, white in dark, dim when inactive). No per-icon CSS.
 *  - No extra network load, no baseline-alignment drift like emoji/icon-fonts.
 *  - Matches this codebase's "inline everything" component style.
 *
 * Usage:
 *   <app-icon name="truck" />
 *   <app-icon name="scale" [size]="18" [stroke]="1.75" />
 *
 * All paths are Lucide (https://lucide.dev, ISC license), 24×24 viewBox,
 * round caps/joins. To add an icon: drop its inner markup into ICONS below.
 */

/** Inner SVG markup per icon, keyed by Lucide name. */
const ICONS: Record<string, string> = {
  ...
  // ── Common UI (ready for the module sweep) ──
  ...
  'package-check': '<path d="M16 16h6"/><path d="M21 10V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l2-1.14"/><path d="M16.5 9.4 7.55 4.24"/><polyline points="3.29 7 12 12 20.71 7"/><line x1="12" x2="12" y1="22" y2="12"/>',
...
@Component({
  ...
  template: `<span class="ph-icon" [innerHTML]="svg()"></span>`,
  styles: `
    :host { display: inline-flex; line-height: 0; }
    .ph-icon { display: inline-flex; }
    .ph-icon ::ng-deep svg { display: block; }
  `,
})
export class Icon {
  private san = inject(DomSanitizer);

  /** Lucide icon name (key of ICONS). Unknown names render nothing. */
  name = input.required<string>();
  /** Pixel size of the square glyph. */
  size = input<number>(20);
  /** SVG stroke width. 1.75 reads crisp at sidebar sizes. */
